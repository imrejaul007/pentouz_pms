...
const createSampleTransactions = async () => {
  try {
    ...
    // Add each transaction directly to the array
    for (const transactionData of sampleTransactions) {
      bankAccount.transactions.push(transactionData);
      console.log(`  ✅ Added: ${transactionData.description} - ₹${(transactionData.creditAmount || transactionData.debitAmount).toLocaleString()}`);
    }
    
    // Update the current balance to the last transaction balance
    bankAccount.currentBalance = sampleTransactions[sampleTransactions.length - 1].balance;
